...
class ContentAction extends AdministratorAction
{
    ...
    {
        $errorinfo = '';
        if (!empty($_POST['hidSubmit'])) {
            $map['iv_name'] = trim($_POST['name']);
            $map['iv_content'] = trim($_POST['content']);
            $map['iv_startdt'] = strtotime(trim($_POST['startdt']));
            $map['iv_enddt'] = strtotime(trim($_POST['enddt']));
            $map['iv_state'] = 1;
            $map['iv_object'] = trim($_POST['objectid']);
            $map['iv_crt'] = time();

            $result = model('Interview')->addInterView($map);
