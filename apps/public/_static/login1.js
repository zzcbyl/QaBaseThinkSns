...
$(document).ready(function() { 
    $('#ajax_login_form1').submit(function(){
        $(this).ajaxSubmit({
    ...
    var checkLoginForm = function() {
        if($('#account_input1').val().length == 0) {
            $('#account_input1').focus();
            return false;
        }
        if($('#pwd_input1').val().length == 0) {
            $('#pwd_input1').focus();
            return false;
        }
        return true;
    ...
    var loginCallback = function(i) {
        // var i = eval("("+e+")");
        if(i.status==1){
            $('#js_login_input1').html('<p>'+i.info+'</p>').show();    
            if(i.data==0){
