                ...
                <div id="col5" class="st-index-main">
	                <div class="extend-foot">
	                    <div class="feed-nav clearfix">
	                    	<div class="my-feed-title"><h4>我的粉丝(共<span id="follow_count" event-node="follower_count" event-args="uid=<?php echo ($GLOBALS['ts']['mid']); ?>"><?php echo (($followerCount)?($followerCount):0); ?></span>人)</h4></div>
		                </div>
						<div class="sperson-feedlist">
						<?php if($data){ ?>
							<?php if(is_array($data)): ?><?php $i = 0;?><?php $__LIST__ = $data?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><dl class="feed_list" id="follower_<?php echo ($vo['uid']); ?>">
									<dt class="face">
										<a href="<?php echo ($vo["space_url"]); ?>" title="<?php echo ($vo["uname"]); ?>"><img src="<?php echo ($vo["avatar_small"]); ?>" /></a>
									</dt>
									<dd class="person-info">
										<div class="info-right right">
											<p><?php echo W('FollowBtn', array('fid'=>$vo['uid'], 'uname'=>$vo['uname'], 'follow_state'=>$vo['followState']));?></p>
											<p><a rel="add" onclick="core.plugInit('blacklist',this,'btn',<?php echo ($vo['uid']); ?>,'')" href="javascript:void(0)">加入黑名单</a></p>
										</div>
									   <div class="info-left">
										<p class="hd">
											<span style="font-size:14px"><?php echo ($vo["space_link"]); ?></span>
											<?php if(is_array($userGroupData[$vo['uid']])): ?><?php $i = 0;?><?php $__LIST__ = $userGroupData[$vo['uid']]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$v1): ?><?php ++$i;?><?php $mod = ($i % 2 )?><img style="width:auto;height:auto;display:inline;cursor:pointer;vertical-align:-3px" src="<?php echo ($v1['user_group_icon_url']); ?>" title="<?php echo ($v1['user_group_name']); ?>" />&nbsp;<?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
										</p>
										<p class="area"><span><?php if($vo["sex"] == 1): ?><i class="ico-male"></i><?php else: ?><i class="ico-female"></i><?php endif; ?>&nbsp;<?php echo ($vo["location"]); ?></span>
										</p>
										<p class="action">											
											<?php echo L('PUBLIC_FOLLOWING');?>：<a href="<?php echo U('public/Profile/following', array('uid'=>$vo['uid']));?>"><?php echo (($vo["following_count"])?($vo["following_count"]):0); ?></a>
											<i class="vline">|</i>
											<?php echo L('PUBLIC_FOLLOWERS');?>：<a href="<?php echo U('public/Profile/follower', array('uid'=>$vo['uid']));?>"><?php echo (($vo["follower_count"])?($vo["follower_count"]):0); ?></a>
											<i class="vline">|</i>
											<?php echo L('PUBLIC_WEIBO');?>：<a href="<?php echo U('public/Profile/index', array('uid'=>$vo['uid']));?>"><?php echo (($vo["feed_count"])?($vo["feed_count"]):0); ?></a>
										</p>
										<?php if($vo['intro']){ ?>
										<p class="info">
											<?php echo L('PUBLIC_PROFILESET_INTRO');?>：<?php echo ($vo["intro"]); ?>
										</p>
										<?php } ?>
										<?php if($user_tag[$vo['uid']]){ ?>
										<p class="info">
											<?php echo L('PUBLIC_TAG');?>：<?php if(is_array($user_tag[$vo['uid']])): ?><?php $i = 0;?><?php $__LIST__ = $user_tag[$vo['uid']]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$u_t_v): ?><?php ++$i;?><?php $mod = ($i % 2 )?><a href="<?php echo U('public/Search/index',array('t'=>3,'a'=>'public','k'=>$u_t_v));?>"><?php echo ($u_t_v); ?></a>&nbsp;&nbsp;&nbsp;<?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
										</p>
										<?php } ?>
									   </div>	
									</dd>
