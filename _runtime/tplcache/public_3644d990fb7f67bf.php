...
<div id="page-wrap">
	<div class="reg-wrap">
		<!--<div class="title"><?php echo L('PUBLIC_FOLLOW_THE_PEOPLE_INTERESTED');?></div>-->
		<div class="reg-main">
			<!--注册步骤-->
			<div class="article article-avatar boxShadow">
				<div class="item-step4">
					<div class="step">
						<span class="step1"></span><span class="step2"></span><span class="step3 step3-current"></span>
					</div>
				<!-- 推荐关注用户 -->
				<?php if($related_recommend_user){ ?>
				<div class="tab-menu">
					<?php if(count($related_recommend_user)==8){ ?>
                		<a event-node="change_related_user" keywords="recommend" type="5" href="<?php echo U('public/Register/getRelatedUser');?>" class="right mt5"><?php echo L('PUBLIC_CHANGE_USERS');?></a>	
                	<?php } ?>				
					<ul>
						<li class="current"><a href="javascript:;">推荐关注用户</a></li>
					</ul>
				</div>
				<div class="face_name_list">
					<ul id="recommend"  model-node="related_user_list">
					<?php if(is_array($related_recommend_user)): ?><?php $i = 0;?><?php $__LIST__ = $related_recommend_user?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$v1): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li><div style="position:relative;width:80px;height:80px">
								<div class="selected"><i class="ico-empty"></i></div>
						  		<a event-node="bulkDoFollowData" value="<?php echo ($v1["userInfo"]["uid"]); ?>" class="face_part_recommend face_part" href="javascript:void(0);">
						  			<img src="<?php echo ($v1["userInfo"]["avatar_big"]); ?>" />
						  		</a>
					  		</div>
					  		<span class="name"><?php echo ($v1["userInfo"]["uname"]); ?></span>
						</li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
					</ul>
					<p class="face_att"><label for="select_all_follow"><input id="select_recommend_follow" event-node="selectAllFollow" keyword="recommend" type="checkbox" class="s-ck" /><?php echo L('PUBLIC_SELECT_ALL');?></label></p>
				</div>
				<?php } ?>

				<!-- TA们跟你拥有相同的标签 -->
				<?php if($related_tag_user){ ?>
				<div class="tab-menu">
					<?php if(count($related_tag_user)==8){ ?>
                		<a event-node="change_related_user" keywords="tag" type="4" href="<?php echo U('public/Register/getRelatedUser');?>" class="right"><?php echo L('PUBLIC_CHANGE_USERS');?></a>	
                	<?php } ?>					
					<ul>
						<li class="current"><a href="javascript:;">TA们跟你拥有相同的标签</a></li>
					</ul>
				</div>
				<div class="face_name_list">
					<ul id="tag" model-node="related_user_list">
					<?php if(is_array($related_tag_user)): ?><?php $i = 0;?><?php $__LIST__ = $related_tag_user?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$v2): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li><div style="position:relative;width:80px;height:80px">
								<div class="selected"><i class="ico-empty"></i></div>
						  		<a event-node="bulkDoFollowData" value="<?php echo ($v2["userInfo"]["uid"]); ?>" class="face_part_tag face_part" href="javascript:void(0);">
						  			<img src="<?php echo ($v2["userInfo"]["avatar_big"]); ?>" />
						  		</a>
					  		</div>
					  		<span class="name"><?php echo ($v2["userInfo"]["uname"]); ?></span>
						</li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
					</ul>
					<p class="face_att"><label for="select_all_follow"><input id="select_tag_follow" event-node="selectAllFollow" keyword="tag" type="checkbox" class="s-ck" /><?php echo L('PUBLIC_SELECT_ALL');?></label></p>
				</div>
				<?php } ?>

				<!-- TA们跟你在同一个地区 -->
				<?php if($related_city_user){ ?>
				<div class="tab-menu">
					<?php if(count($related_city_user)==8){ ?>
                		<a event-node="change_related_user" keywords="city" type="3" href="<?php echo U('public/Register/getRelatedUser');?>" class="right"><?php echo L('PUBLIC_CHANGE_USERS');?></a>	
                	<?php } ?>					
					<ul>
						<li class="current"><a href="javascript:;">TA们跟你在同一个地区：<?php echo ($location); ?></a></li>
					</ul>
				</div>
				<div class="face_name_list">
					<ul id="city" model-node="related_user_list">
					<?php if(is_array($related_city_user)): ?><?php $i = 0;?><?php $__LIST__ = $related_city_user?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$v3): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li><div style="position:relative;width:80px;height:80px">
								<div class="selected"><i class="ico-empty"></i></div>
						  		<a event-node="bulkDoFollowData" value="<?php echo ($v3["userInfo"]["uid"]); ?>" class="face_part_city face_part" href="javascript:void(0);">
						  			<img src="<?php echo ($v3["userInfo"]["avatar_big"]); ?>" />
						  		</a>
					  		</div>
					  		<span class="name"><?php echo ($v3["userInfo"]["uname"]); ?></span>
						</li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
					</ul>
					<p class="face_att"><label for="select_all_follow"><input id="select_city_follow" event-node="selectAllFollow" keyword="city" type="checkbox" class="s-ck" /><?php echo L('PUBLIC_SELECT_ALL');?></label></p>
				</div>
				<?php } ?>
				
				<div class="actionBtn mt15 clearfix">
					<a class="btn-register left mr15" event-node="saveFollow" href="<?php echo U('public/Register/bulkDoFollow');?>"><?php echo L('PUBLIC_SAVE');?></a>
					<a href="<?php echo U('public/Register/doStep4');?>" class="left mt15"><span><?php echo L('PUBLIC_BACKUP');?>&raquo;</span></a>
				</div>
				</div>
			</div>
			<!--//注册步骤-->
