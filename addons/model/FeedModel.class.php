...
class FeedModel extends Model
{
    ...
    public function getList($map, $limit = 10, $order = 'publish_time desc')
    {
        ...
        foreach ($feedlist["data"] as $v => $vv) {
            ...
            //附件
            $feed_data = unserialize($vv['feed_data']);
    ...
    public function getFollowingFeed($where = '', $limit = 10, $uid = '', $fgid = '', $LoadWhere = '')
    {
        ...
        //关注的问题
        $table .= " union (SELECT feedid, ctime FROM `wb_feed_following` WHERE `uid` ={$buid} )";
    ...
    public function getCommentFeedList($where = '', $limit = 10, $LoadWhere = '')
    {
        ...
        if (is_array($feedlist["data"]) && count($feedlist["data"]) > 0) {
            foreach ($feedlist["data"] as $v => $vv) {
                ...
                //附件
                $feed_data = unserialize($Answer[0]['feed_data']);
                if (!empty($feed_data['attach_id'])) {
                    $attachInfo = model('Attach')->getAttachByIds($feed_data['attach_id']);
                    $_attach = array();
                    $index = 0;
                    foreach ($attachInfo as $ak => $av) {
                        ...
                        if (trim($av['extension']) == 'mp3') {
                            ...
                            $Answer[0]["attach_mp3"][$index] = $_attach;
                            $index++;
                        }
                    }
                }
