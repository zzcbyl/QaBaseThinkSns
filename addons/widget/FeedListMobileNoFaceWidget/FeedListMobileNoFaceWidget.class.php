    ...
    private function getData($var, $tpl = 'FeedList.html') {
		...
		switch($type) {
			...
			case 'question':// 我问题
				...
				$where =" is_del = 0 AND feed_questionid=0 AND add_feedid=0 AND (is_audit=1 OR is_audit=0) ".$LoadWhere." and `openid`= '".$var['openid']."'";
				$list = model('Feed')->getQuestionAndAnswer($where, $this->limitnums);
				//print($where);
				break;
			case 'lls_answer':	//卢老师回答列表
				...
				$where =" is_del = 0 AND feed_questionid!=0 AND add_feedid=0 AND (is_audit=1 OR is_audit=0) ".$LoadWhere." and `openid`= '".$var['expert']."'";
