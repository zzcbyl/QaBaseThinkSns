...
core.comment_mobile = {
    ...
    _init: function (attrs) {
        if (attrs.length == 3) {
            core.comment_mobile.init(attrs[1], attrs[2]);
        } else {
            return false;
        }
    },
    ...
    display: function (isshow) {
        ...
        $.post(U('widget/CommentMobile/render'), { app_uid: this.app_uid, row_id: this.row_id, app_row_id: this.app_row_id, app_row_table: this.app_row_table, isAjax: 1, showlist: 0,
            ...
            if (html.status == '0') {
                ...
                $.post(U('widget/CommentMobile/getCommentList'), { app_name: appname, table: table, row_id: rowid, cancomment: cancomment, comment_type: commenttype, random: Math.random() }, function (res) {
                ...
                commentListObj.innerHTML = html.data;
                $('#commentlist_' + rowid).html('<img src="' + THEME_URL + '/image/load.gif" style="text-align:center;display:block;margin:0 auto;"/>');
                $.post(U('widget/CommentMobile/getCommentList'), { app_name: appname, table: table, row_id: rowid, cancomment: cancomment, comment_type: commenttype }, function (res) {
                    $('#commentlist_' + rowid).html(res);
                    M($('#commentlist_' + rowid).get(0));
                });
                M(commentListObj);
    ...
    addComment: function (afterComment, obj) {
        ...
        var _this = this;
        //obj.innerHTML = '回复中..';
        $.post(U('widget/CommentMobile/addcomment'), {
    ...
    delComment: function (comment_id) {
        var commentListObj = this.commentListObj;
        $.post(U('widget/CommentMobile/delcomment'), { comment_id: comment_id }, function (msg) {
            //什么也不做吧
