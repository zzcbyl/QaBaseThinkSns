...
core.weibo_mobile = {
    ...
    init: function (agrs) {
        ...
        //			core.weibo_mobile.loadMoreFeed();
        //		}
    },
    // 页底加载提问
    bindScroll: function () {
            ...
            if (bodyTop + $(window).height() >= bodyHeight - 250) {
                ...
                if ($('#feed-lists').length > 0 && _this.loadId != null) {
                    ...
                    if (this.answerid != null && this.answerid != undefined) {
                        core.weibo_mobile.loadMoreAnswer(this.answerid); //加载答案
                    }
                    else {
                        core.weibo_mobile.loadMoreFeed();
                    }
                    ...
                    if ($('#feed-lists .page').find('a').size() > 2) {
                        ...
                        $('#feed-lists .page').find('a').each(function () {
                            ...
                            if (href) {
                                ...
                                $(this).click(function () {
                                    core.weibo_mobile.loadMoreByPage(href);
                                });
                            }
                        });
                    } else {
    ...
    // 发布提问
    post_feed: function (_this, mini_editor, textarea, description_editor, description, questionid, isbox, url, isAdd, inviteList, openid, isnoname) {
        var obj = this;
        // 避免重复发送
        ...
        if (openid)
            Oid = openid.value;
        // 发布提问
        $.post(url, { body: data, type: type, app_name: app_name, content: '', attach_id: attach_id, videourl: videourl, channel_id: channel_id, source_url: attrs.source_url, gid: attrs.gid, description: txtVal, questionid: Qid, addask: int_isadd, inviteid: inviteid, ShareSina: sinaShare, ShareQQ: qqShare, Openid: Oid, IsNoName: isnoname }, function (msg) {
            //alert(msg.data);
            obj.isposting = false;
            ...
            if (msg.status == 1) {
                ...
                $(postOk).fadeIn('fast');
                core.weibo_mobile.afterPost(mini_editor, textarea, attrs.topicHtml, description_editor, description, false, Qid, int_isadd);
                if (!isbox) {
                    core.weibo_mobile.insertToList(msg.data, msg.feedId, questionid, int_isadd);
                } else {
                    ui.box.close();
                ...
                if ("undefined" != typeof (inviteList) && inviteList.value != "") {
                    core.weibo_mobile.Invite(msg.feedId, inviteList.value);
                }
