...
if("undefined" == typeof(core.topic)){	//只init一次
	core.plugFunc('topic',function(){
		core.topic.init(args);	
	});
}
/**
 * 事件绑定器
...
M.addEventFns({
    ...
    post_feed: {	//发布普通|图片提问 
        click: function () {
            ...
            core.topic.post_feed(_this, mini_editor, textarea, description_editor, description, questionid, false, '', 0, inviteList);
        }
    },
    post_addask: {	//追问
        click: function () {
            ...
            var questionid_editor = this.parentModel.parentModel.childModels['mini_editor'][0];
            var questionid = $(mini_editor).find('input').get(0);
            core.topic.post_feed(_this, mini_editor, textarea, null, null, questionid, false, "", true);
        }
    },
...
//            alert(questionid.value);
    ...
    post_feed_box: {
        click: function () {
            ...
            var mini_editor = this.parentModel.parentModel.childModels['mini_editor'][0];
            var textarea = $(mini_editor).find('textarea').get(0);
            core.topic.post_feed(_this, mini_editor, textarea, true);
        }
    },
    post_submission_box: {
        click: function () {
            ...
            var mini_editor = this.parentModel.parentModel.childModels['mini_editor'][0];
            var textarea = $(mini_editor).find('textarea').get(0);
            core.topic.post_feed(_this, mini_editor, textarea, true, 'submission');
        }
    },
    insert_at: {
        click: function () {
                    ...
                    if ("undefined" != typeof (args.t)) {
                        ...
                        switch (args.t) {
                            ...
                            case 'feed':
                                ...
                                core.topic.post_feed(postObj, this.parentModel, _this);
                                break;
                            case 'repostweibo':
